    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <el-select
          ...
          @change="crud.toQuery"
        >
          <el-option v-for="item in dict.switch_status_1_0" :key="item.id" :label="item.label" :value="item.value" />
        </el-select>
      ...
      >
        ...
        <el-table-column
          v-if="checkPer(['admin', 'nationCode:edit', 'nationCode:del'])"
          label="操作"
          width="150px"
          align="center"
        >
...
<script>
  ...
  export default {
    ...
    cruds() {
      return CRUD({
        title: '国家编码',
        url: 'api/nationCode/query',
        idField: 'id',
        sort: 'id,desc',
        crudMethod: { ...crudNationCode },
      })
    },
    data() {
      return {
        ...
        rules: {
          code: [{ required: true, message: '国家编码2位不能为空', trigger: 'blur' }],
          alpha3Code: [{ required: true, message: '国家编码3位不能为空', trigger: 'blur' }],
          numeric: [{ required: true, message: '国家数字编码不能为空', trigger: 'blur' }],
          englishName: [{ required: true, message: '国家名称（英文）不能为空', trigger: 'blur' }],
          chineseName: [{ required: true, message: '国家名称（中文）不能为空', trigger: 'blur' }],
          currency: [{ required: true, message: '货币不能为空', trigger: 'blur' }],
          phoneAreaCode: [{ required: true, message: '手机区号不能为空', trigger: 'blur' }],
          flag: [{ required: true, message: '国旗不能为空', trigger: 'blur' }],
          enabled: [{ required: true, message: '状态：1启用、0禁用不能为空', trigger: 'blur' }],
        },
        queryTypeOptions: [
